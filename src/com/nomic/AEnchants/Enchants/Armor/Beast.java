...
public class Beast implements Listener {
	...
	public void shift(ArmorShift e) {
		...
		String one = names.getConfigurationSection("Beast").getString("one");
		String two = names.getConfigurationSection("Beast").getString("two");
		String three = names.getConfigurationSection("Beast").getString("three");
		...
		if (m == diamondc || m == ironc || m == chainc || m == leatherc) {
			if (stype == SlotType.ARMOR && inv.getChestplate() != null) {
				...
				if (inv.getHelmet() == null || inv.getLeggings() == null || inv.getBoots() == null)
					return;
				if (!(inv.getHelmet().hasItemMeta() || inv.getLeggings().hasItemMeta() || inv.getBoots().hasItemMeta()))
					return;
			...
			if (stype == SlotType.ARMOR && inv.getLeggings() != null) {
				...
				if (inv.getHelmet() == null || inv.getChestplate() == null || inv.getBoots() == null)
					return;
				if (!(inv.getHelmet().hasItemMeta() || inv.getChestplate().hasItemMeta()
						|| inv.getBoots().hasItemMeta()))
					return;
				List<String> hlore = inv.getHelmet().getItemMeta().getLore();
			...
			if (stype == SlotType.ARMOR && inv.getBoots() != null) {
				...
				if (inv.getHelmet() == null || inv.getLeggings() == null || inv.getChestplate() == null)
					return;
				if (!(inv.getHelmet().hasItemMeta() || inv.getLeggings().hasItemMeta()
						|| inv.getChestplate().hasItemMeta()))
					return;
				List<String> hlore = inv.getHelmet().getItemMeta().getLore();
				List<String> clore = inv.getChestplate().getItemMeta().getLore();
				List<String> llore = inv.getLeggings().getItemMeta().getLore();
			...
			if (stype == SlotType.ARMOR && inv.getHelmet() != null) {
				...
				if (inv.getChestplate() == null || inv.getLeggings() == null || inv.getBoots() == null)
					return;
				if (!(inv.getChestplate().hasItemMeta() || inv.getLeggings().hasItemMeta() || inv.getBoots().hasItemMeta()))
					return;
				List<String> clore = inv.getChestplate().getItemMeta().getLore();
				List<String> llore = inv.getLeggings().getItemMeta().getLore();
				List<String> blore = inv.getBoots().getItemMeta().getLore();
	...
	public void hotbar(ArmorHotbar e) {
		...
		if (m == diamondb || m == ironb || m == chainb || m == leatherb) {
			if (inv.getBoots() == null) {
				if (inv.getHelmet() == null || inv.getLeggings() == null || inv.getChestplate() == null)
					return;
				if (!(inv.getHelmet().hasItemMeta() || inv.getLeggings().hasItemMeta() || inv.getChestplate().hasItemMeta()))
					return;
				List<String> hlore = inv.getHelmet().getItemMeta().getLore();
				List<String> llore = inv.getLeggings().getItemMeta().getLore();
				List<String> clore = inv.getChestplate().getItemMeta().getLore();
				if (lore.contains(tOne)) {
					if (!(clore.contains(tOne) && llore.contains(tOne) && hlore.contains(tOne)))
						return;
					p.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 1000000 * 20, 0));
				} else if (lore.contains(tTwo)) {
					if (!(clore.contains(tTwo) && llore.contains(tTwo) && hlore.contains(tTwo)))
						return;
					p.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 1000000 * 20, 0));
				} else if (lore.contains(tThree)) {
					if (!(clore.contains(tThree) && llore.contains(tThree) && hlore.contains(tThree)))
						return;
					p.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 1000000 * 20, 1));
				}
			}
